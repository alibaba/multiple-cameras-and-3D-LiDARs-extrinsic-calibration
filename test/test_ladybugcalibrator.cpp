...
int main(int argc, char **argv)
{
    if (argc < 9)
    {
        std::cerr << "Usage : test_ladybugcalibrator [global_map.yaml] [cctag_result.yaml]\n \
        [cam0_intrin_file] [cam1_intrin_file] [cam2_intrin_file] [cam3_intrin_file] [cam4_intrin_file]\n \
        [output_folder] \n";
        return -1;
    }
    // start timer
    ...
    std::string cam4_calibration_file_name(argv[7]);
    std::string output_folder(argv[8]);
    ...
    std::vector<std::string> v_intrinsic_fns = {cam0_calibration_file_name, cam1_calibration_file_name,
                                                cam2_calibration_file_name, cam3_calibration_file_name, cam4_calibration_file_name};
    ...
    LadybugCalibrator ladybug_calibrator(camera::model_type_t::Fisheye, target::target_type_t::CCTAG, v_intrinsic_fns, gt_map_file_name, 5);
    sts = ladybug_calibrator.addCctagData(v_calib_img_paths, v_cctag_centers);
    if (!sts)
    {
    ...
    sts = ladybug_calibrator.calibrate();
    if (!sts)
    {
    ...
    // TIMER_STREAM("[test_LadybugCalibrator] caliration time is " << calib_time << " s");

    std::vector<Eigen::Matrix4d> v_cam_poses_w2c = ladybug_calibrator.validCameraPoses();
    // rotation axis 2 camera
    std::vector<Eigen::Matrix4d> v_cam_poses_r2c = ladybug_calibrator.validCameraPosesInRotationAxisFrame();
    ...
    for (size_t i = 0; i < v_cam_poses_w2c.size(); ++i)
    {
        ...
        std::string yml_filepath = output_folder + "/camera0_to_camera"+ std::to_string(i) +".yml";
    ...
    //         if (!sts)
    //         {
    //             ERROR_STREAM("[test_LadybugCalibrator] Fail to write frame " << i);
    //             return -1;
    //         }
    //     }
    // }
...
#ifdef VISUALIZE_TRAJECTORY
    std::vector<Eigen::Matrix4d> v_visual_frames = v_cam_poses_w2c;
    Viewer viewer(v_visual_frames);
    std::vector<Eigen::Vector3d> v_map_points = ladybug_calibrator.objectPoints();
