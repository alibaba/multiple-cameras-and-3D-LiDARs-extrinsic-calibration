...
#include "ladybug5_calibration.h"

LadybugCalibrator::LadybugCalibrator(camera::model_type_t camModelType,
...
int LadybugCalibrator::readImageWidth(const std::string &filename)
{
    if (filename.empty())
    {
        ERROR_STREAM("[LadybugCalibrator::readImageWidth] Empty file name!");
        return -1;
    }
...
int LadybugCalibrator::readImageHeight(const std::string &filename)
{
    if (filename.empty())
    {
        ERROR_STREAM("[LadybugCalibrator::readImageHeight] Empty file name!");
        return -1;
    }
...
bool LadybugCalibrator::addCctagData(const std::vector<std::string> &imgFilePaths, const std::vector<std::vector<cv::Point2d>> &centers)
{
    ...
    for (size_t i = 0; i < imgFilePaths.size(); ++i)
    {
        ...
        if (!sts)
        {
            ERROR_STREAM("[LadybugCalibrator::addCctagData] Failed to add cctag data in left camera!");
            return false;
        }
    }
...
const std::vector<Eigen::Vector3d> LadybugCalibrator::objectPoints() const
{
    return v_object_points_;
}

std::vector<Eigen::Matrix4d> LadybugCalibrator::validCameraPoses() const
{
...
std::vector<Eigen::Matrix4d> LadybugCalibrator::validCameraPosesInRotationAxisFrame() const
{
    DEBUG_STREAM("[LadybugCalibrator::validCameraPosesInRotationAxisFrame] Get " << v_cam_pose_r_c_.size()
...
bool LadybugCalibrator::calibrate()
{

    for (size_t i = 0; i < cam_num_; ++i)
    {
        DEBUG_STREAM("[LadybugCalibrator::calibrate] ---------------- calibrate camera " << i << "------------------");
        if (!v_cam_calibrator_ptrs_[i]->calibrate())
        {
            ERROR_STREAM("[LadybugCalibrator::calibrate] Fail to calibrate camera " << i);
            return false;
        }
    }
    ...
    for (size_t i = 0; i < v_cam_calibrator_ptrs_.size(); ++i)
    {
        ...
        DEBUG_STREAM("[LadybugCalibrator::calibrate] rotation axis : " << rot_i_0_vec.axis().transpose());
        DEBUG_STREAM("[LadybugCalibrator::calibrate] rotation angle : " << opt_rotation_angles[i] * RAD_2_DEGREE);
    }
    ...
    if (!sts)
    {
        ERROR_STREAM("[LadybugCalibrator::calibrate] Fail to optimize");
        return false;
    }
    ...
    for (size_t i = 0; i < v_cam_calibrator_ptrs_.size(); ++i)
    {
        ...
        // check inliers after optimization
        {
            ...
            for (size_t j = 0; j < 1; ++j)
            {
                ...
                {
                    sts = false;
                    ERROR_STREAM("[LadybugCalibrator::calibrate] Optimize camera " << i << " failed!");
                }

                if (i <= cam_num_ / 2)
                {
                    ERROR_STREAM("[LadybugCalibrator::calibrate] Camera " << i << " rotation angle " << motor_rot_vec.angle() * RAD_2_DEGREE);
                }
                else
                {
                    ERROR_STREAM("[LadybugCalibrator::calibrate] Camera " << i << " rotation angle " << (motor_rot_vec.angle() + 2 * M_PI) * RAD_2_DEGREE);
                }

                ERROR_STREAM("[LadybugCalibrator::calibrate] Frame " << i << " Original inlier num: " << origin_inlier_num);
                ERROR_STREAM("[LadybugCalibrator::calibrate] Frame " << i << " Optimized inlier num: " << tmp_inlier_num);
                ERROR_STREAM("[LadybugCalibrator::calibrate] Frame " << i << " Original mean reprojection error : " << origin_mean_reproject_error);
                ERROR_STREAM("[LadybugCalibrator::calibrate] Frame " << i << " Optimized mean reprojection error : " << mean_reproject_error);
            }
        }
        ...
        if (relative_rotation_angle < -M_PI)
            relative_rotation_angle += 2 * M_PI;
        ERROR_STREAM("[LadybugCalibrator::calibrate] relative rotation angle " << relative_rotation_angle * RAD_2_DEGREE);
    }

    DEBUG_STREAM("[LadybugCalibrator::calibrate] motor axis: " << motor_axis_vector.transpose());
    DEBUG_STREAM("[LadybugCalibrator::calibrate] global axis: " << global_rotation_axis_.transpose());
    DEBUG_STREAM("[LadybugCalibrator::calibrate] delta vector to axis center : " << delta_0_vector.transpose() << ", norm : " << delta_0_vector.norm());

    return calcCameraPoser2c(global_rotation_axis_);
}

bool LadybugCalibrator::calcRotationAxisCenter(const std::vector<MonoCalibrator::Ptr> &v_calib_ptrs, Eigen::Vector3d &axis_center)
{
    ...
    axis_center[0] /= cam_num_;
    axis_center[1] /= cam_num_;
    axis_center[2] /= cam_num_;
    std::cout << "[calcRotationAxisCenter] axis_center : " << axis_center.transpose() << std::endl;
    return true;
}

bool LadybugCalibrator::calcCameraPoser2c(const Eigen::Vector3d &global_rotation_axis)
{
    if (global_rotation_axis.hasNaN())
    {
        ERROR_STREAM("[LadybugCalibrator::calCameraPoser2c] INvalid input!");
        return false;
    }
    ...
    // calculate relative pose between 4 cameras
    {
        ...
        if (!sts)
        {
            ERROR_STREAM("[LadybugCalibrator::calCameraPoser2c] Fail to calculate global_rotation_center");
            return false;
        }
        ...
        Eigen::Matrix4d T_w_rotaionaxis = Eigen::Matrix4d::Identity();
        {
            ...
            DEBUG_STREAM("[LadybugCalibrator::calCameraPoser2c] axis_x : " << axis_x.transpose());
            DEBUG_STREAM("[LadybugCalibrator::calCameraPoser2c] axis_y : " << axis_y.transpose());
            DEBUG_STREAM("[LadybugCalibrator::calCameraPoser2c] axis_z : " << axis_z.transpose());
